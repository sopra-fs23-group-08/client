...
const Game3 = () => {
    ...
    const [roundWinnerToken, setRoundWinnerToken] = useState("")
    ...
    const handlePlayersUpdate = (message) => {
        ...
        if(isMounted.current) {
            ...
            // set currentPlayer if the currentPlayer is true for the player object in player list
            setCurrentPlayer(playerArray.find((p) => p.currentPlayer === true))
        }
    }
    const handleGameStateUpdate = (message) => {
        ...
        if(isMounted.current) {
            ...
            setHasStarted(gameState.hasStarted)
            setRoundWinnerToken(gameState.roundWinnerToken)
        }
    }
